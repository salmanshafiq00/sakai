...
import { AppUserModel, CreateRoleCommand, RolesClient, TreeNodeListsClient, UpdateRoleCommand, RoleModel, TreeNodeModel } from 'src/app/modules/generated-clients/api-service';
...
export class RoleDetailComponent {
  ...
  private getById(id: string) {
    this.entityClient.getRole(id).subscribe({
      next: (res: RoleModel) => {
        ...
        this.selectNodes(this.optionDataSources['permissionNodeList'], res.permissions)
        this.updateParentSelection(this.optionDataSources['permissionNodeList'])
  ...
  updateParentSelection(nodes: TreeNode[]) {
    nodes.forEach(node => {
      if (node.children && node.children.length > 0) {
        this.updateParentSelection(node.children);  // Recursively update child nodes
        const totalChildren = node.children.length;
        const selectedChildren = node.children.filter(child => this.selectedPermissions.includes(child)).length;
        if (selectedChildren === totalChildren) {
          ...
          const parentIndex = this.selectedPermissions.indexOf(node);
          if (parentIndex !== -1) {
            this.selectedPermissions.splice(parentIndex, 1);
          }
        } else {
